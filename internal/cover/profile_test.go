		...
		for _, xor := range []bool{false, true} {
			for _, clear := range []bool{false, true} {
				for _, mux := range []bool{false, true} {
					want := Profile{Enabled: true, ID: uint16(id), Padding: uint16(id % (MaxPadding + 1)), HeaderMask: byte(id), XOR: xor, Clear: clear, MuxV2: mux}
					...
					if !ok || got != want {
						t.Fatalf("id=%04x xor=%t clear=%t mux=%t got=%+v ok=%t", id, xor, clear, mux, got, ok)
					}
				}
			}
		}
